// ═══════════════════════════════════════════════════════════════════════════
// STARRY NIGHT V38 - HIGH-END MECHANICAL ART
// All corrections from V37 feedback
// ═══════════════════════════════════════════════════════════════════════════
$fn = 64;
...
// ═══════════════════════════════════════════════════════════════════════════
// CLIFF DIMENSIONS - Height reduced by 20mm
// ═══════════════════════════════════════════════════════════════════════════
CLIFF_WIDTH = 165;
CLIFF_RECT_WIDTH = 100;
CLIFF_HEIGHT = 85;  // Was 105, now 85 (-20mm)

// ═══════════════════════════════════════════════════════════════════════════
// WIND PATH - Positioned to touch left edge, brought to FOREGROUND
// Shape bounds: X[-892, 892], Y[-267, 267]
// ═══════════════════════════════════════════════════════════════════════════
WIND_SCALE = 0.165;
WIND_ORIG_MIN_X = -892.309;
WIND_TRANSLATE_X = -WIND_ORIG_MIN_X * WIND_SCALE;  // 147.2mm
WIND_TRANSLATE_Y = IH * 0.55;
...
BIG_SWIRL_R = BIG_CUTOUT_R - 1.8 + 1.5;   // 41.7mm
SMALL_SWIRL_R = SMALL_CUTOUT_R - 1.8;      // 28.2mm

// ═══════════════════════════════════════════════════════════════════════════
// CYPRESS - Move left 100mm, 30% bigger, ensure no gap at bottom
// Original scale was 0.82, now 0.82 * 1.3 = 1.066
// ═══════════════════════════════════════════════════════════════════════════
CYPRESS_SCALE = 1.066;  // 30% bigger than 0.82
CYPRESS_ORIG_MIN_Y = -112.572;
// After scaling, bottom at: -112.572 * 1.066 = -120.0mm
// Translate Y to bring bottom to Y=0 (add small extra to ensure no gap)
CYPRESS_TRANSLATE_Y = -CYPRESS_ORIG_MIN_Y * CYPRESS_SCALE + 2;  // +2mm to ensure contact
// Move left 100mm: was at CLIFF_WIDTH + 20 = 185, now 185 - 100 = 85
CYPRESS_X = 85;

// ═══════════════════════════════════════════════════════════════════════════
// WAVE POSITIONING
// ═══════════════════════════════════════════════════════════════════════════
WAVE_ZONE_START = CLIFF_WIDTH + 15;
WAVE_ZONE_END = IW - 5;
...
// ═══════════════════════════════════════════════════════════════════════════
// Z-LAYERS - Wind path moved to FOREGROUND (before lighthouse)
// ═══════════════════════════════════════════════════════════════════════════
Z_SKY = 0;
Z_BOTTOM_GEARS = 6;
Z_BIRD_SUPPORTS = 8;
...
Z_BELTS = 30;
Z_CLIFFS = 32;
Z_WIND = 38;          // FOREGROUND - before lighthouse (was 32)
Z_LIGHTHOUSE = 40;
Z_WAVE_1 = 42;
Z_WAVE_2 = 45;
Z_WAVE_3 = 48;
...
// ═══════════════════════════════════════════════════════════════════════════
// MOON - Thickness HALVED for all discs
// ═══════════════════════════════════════════════════════════════════════════
module moon_halo_back(rot) {
    rotate([0,0,rot])
    color(C_MOON_HALO_A, 0.5)
    difference() {
        cylinder(r=50, h=1.5, $fn=96);  // Was 3, now 1.5
        translate([0,0,-1]) cylinder(r=42, h=4, $fn=96);
        for(i=[0:7]) rotate([0,0,i*45+22.5]) 
            translate([46,0,-1]) cylinder(r=3.5, h=4);
    }
}
...
module moon_halo_front(rot) {
    ...
    difference() {
        cylinder(r=42, h=1.5, $fn=96);  // Was 3, now 1.5
        translate([0,0,-1]) cylinder(r=34, h=4, $fn=96);
        for(i=[0:5]) rotate([0,0,i*60]) 
            translate([38,0,-1]) cylinder(r=2.8, h=4);
    }
}

module moon_core(rot) {
    color(C_MOON, 0.2) cylinder(r=32, h=1, $fn=96);  // Was 2, now 1
    translate([0,0,1]) color(C_MOON) cylinder(r=20, h=2.5, $fn=96);  // Was 5, now 2.5
    translate([0,0,1]) rotate([0,0,rot])
    color(C_MOON, 0.8) for(r_val=[24, 28, 32]) difference() {
        cylinder(r=r_val+1.5, h=2, $fn=96);  // Was 4, now 2
        translate([0,0,-1]) cylinder(r=r_val-1, h=4, $fn=96);
        for(a=[0:3]) rotate([0,0,a*90+15]) translate([0,0,-1]) cube([40, 2.8, 5]);
    }
}

// ═══════════════════════════════════════════════════════════════════════════
// SWIRL DISC - Thickness HALVED
// ═══════════════════════════════════════════════════════════════════════════
module swirl_halo_back(r, rot) {
    rotate([0,0,rot])
    color(C_SWIRL_GREY_DARK, 0.82)
    difference() {
        cylinder(r=r*1.08, h=1.5, $fn=96);  // Was 3, now 1.5
        translate([0,0,-1]) cylinder(r=r*0.78, h=4, $fn=96);
        for(i=[0:7]) rotate([0,0,i*45+22.5]) 
            translate([r*0.93,0,-1]) cylinder(r=r*0.08, h=4);
    }
}
...
module swirl_halo_front(r, rot) {
    ...
    difference() {
        cylinder(r=r*0.88, h=2, $fn=96);  // Was 4, now 2
...
module swirl_main(r, rot) {
    ...
    difference() {
        cylinder(r=r, h=2.5, $fn=96);  // Was 5, now 2.5
        translate([0,0,-1]) cylinder(r=r*0.07, h=5, $fn=96);
        for(i=[0:2]) rotate([0,0,i*120]) 
            translate([r*0.52,0,-1]) cylinder(r=r*0.12, h=5);
        translate([0,0,1.5])
        difference() {
            cylinder(r=r*0.92, h=2, $fn=96);
            cylinder(r=r*0.78, h=3, $fn=96);
        }
    }
    translate([0,0,2]) color(C_GEAR) cylinder(r=r*0.10, h=1.5);  // Was h=3, now 1.5
}

// ═══════════════════════════════════════════════════════════════════════════
// STAR GEAR - Thickness HALVED for halos
// ═══════════════════════════════════════════════════════════════════════════
module star_gear(r, rot, halo_rot) {
    rotate([0,0,rot]) {
        color(C_STAR) difference() {
            cylinder(r=r, h=2, $fn=64);  // Was 4, now 2
        ...
        color(C_STAR) for(i=[0:7]) rotate([0,0,i*45])
            translate([r*0.75,0,0]) cylinder(r=r*0.12, h=2, $fn=3);  // Was 4, now 2
    }
    translate([0,0,-2]) rotate([0,0,halo_rot])  // Was -4, now -2
    color(C_GEAR_DARK, 0.8) difference() {
        cylinder(r=r*1.55, h=1, $fn=64);  // Was 2, now 1
    ...
    color(C_GEAR, 0.7) difference() {
        cylinder(r=r*1.32, h=1, $fn=64);  // Was 2, now 1
        translate([0,0,-1]) cylinder(r=r*0.92, h=3, $fn=64);
        for(i=[0:7]) rotate([0,0,i*45+22.5]) translate([r*1.12,0,-1]) cylinder(r=r*0.1, h=3);
    }
}

// ═══════════════════════════════════════════════════════════════════════════
// WIND PATH - Now visible in foreground, with circular cutouts
// ═══════════════════════════════════════════════════════════════════════════
module wind_path_with_circular_cutouts() {
    difference() {
        ...
        color(C_WIND)
        linear_extrude(height=5)  // Explicit extrusion
...
// Alternative wind path module without projection (in case above doesn't work)
module wind_path_simple() {
    difference() {
        translate([WIND_TRANSLATE_X, WIND_TRANSLATE_Y, 0])
        scale([WIND_SCALE, WIND_SCALE, 1])
        color(C_WIND)
...
// ═══════════════════════════════════════════════════════════════════════════
// CLIFF - Height now 85mm (was 105, -20mm)
// ═══════════════════════════════════════════════════════════════════════════
module cliff_assembly() {
    ...
    color(C_CLIFF_DARK)
    for(i=[0:3]) {
        translate([5+i*18, CLIFF_HEIGHT*(0.15+i*0.15), 15])
        rotate([0,0,-10+i*4])
...
// ═══════════════════════════════════════════════════════════════════════════
// CYPRESS - Moved left 100mm, 30% bigger, base at Y=0
// ═══════════════════════════════════════════════════════════════════════════
module cypress_at_bottom() {
    // X position moved left 100mm (from 185 to 85)
    // Scale increased 30% (from 0.82 to 1.066)
    // Y translate calculated to bring bottom to Y=0
    translate([CYPRESS_X, CYPRESS_TRANSLATE_Y, 0])
...
// ═══════════════════════════════════════════════════════════════════════════
// CLIFF WAVES - White/foam waves rotated -70° on Z axis (negative)
// ═══════════════════════════════════════════════════════════════════════════
module cliff_waves(drift, surge, crash, tilt) {
    ...
    rotate([0, 0, -70])  // NEGATIVE 70 degrees
    ...
    // Foam spray 2 - ROTATED -70° on Z axis
...
// NOTE: Left gears near lighthouse REMOVED per user question
// ═══════════════════════════════════════════════════════════════════════════
...
// SWIRL DISC 2 (SMALL) - CCW, half thickness
...
// CLIFF (height reduced by 20mm)
translate([FW, FW, Z_CLIFFS]) cliff_assembly();

// WIND PATH - FOREGROUND (Z=38, before lighthouse at Z=40)
translate([FW, FW, Z_WIND]) wind_path_simple();
...
// SKY GEARS (foreground, left gears removed)
...
// ═══════════════════════════════════════════════════════════════════════════
echo("═══════════════════════════════════════════════════════════════════════");
echo("STARRY NIGHT V38 - HIGH-END MECHANICAL ART");
echo("═══════════════════════════════════════════════════════════════════════");
echo("Canvas:", W, "×", H, "mm | Inner:", IW, "×", IH, "mm");
echo("");
echo("FIXES IMPLEMENTED:");
echo("  1. Cliff height:", CLIFF_HEIGHT, "mm (was 105, -20mm)");
echo("  2. Cypress: X=", CYPRESS_X, "(moved left 100mm), Scale=", CYPRESS_SCALE, "(30% bigger)");
echo("  3. All disc thicknesses HALVED");
echo("  4. Wind path at Z=38 (FOREGROUND, before lighthouse Z=40)");
echo("  5. Cliff waves: rotate([0,0,-70])");
echo("  6. Left gears near lighthouse: REMOVED");
echo("");
