// ═══════════════════════════════════════════════════════════════════════════════════════
//                    STARRY NIGHT V48 - COMPLETE ASSEMBLY
//                    Physics-Based Wave System + Articulated Breaking Wave
//                    11-Star Twinkle System + All V47 Components Preserved
// ═══════════════════════════════════════════════════════════════════════════════════════
// VERSION: V48
// BASE: V47 (User's Vision with Clock-Style Gears)
// NEW FEATURES:
//   - 3-Zone Wave System (Far/Mid/Breaking)
//   - Articulated Breaking Wave (multi-segment curl)
//   - 11-Star Twinkle System with counter-rotating halos
//   - Cypress wind sway (+/-3°)
//   - Swirl disc pulse (+/-2mm Z)
// ═══════════════════════════════════════════════════════════════════════════════════════
$fn = 64;

// ═══════════════════════════════════════════════════════════════════════════════════════
//                                MASTER DIMENSIONS (Section 2 - LOCKED)
// ═══════════════════════════════════════════════════════════════════════════════════════
W = 350;              // Total width
H = 275;              // Total height
D = 95;               // Total depth
...
Z_LED = 2;
Z_GEAR_PLATE = 5;
Z_STAR_HALO = 6;        // V48: Star halos behind gears
Z_STAR_GEAR = 10;       // V48: Star gears
Z_MOON_PHASE = 15;
Z_MOON_CRESCENT = 20;
Z_SWIRL_INNER = 25;
Z_SWIRL_GEAR = 28;
...
Z_WAVE_LAYER_T = 4;     // Each wave layer is 4mm thick
Z_CYPRESS = 75;
Z_BIRD_WIRE = 82;
Z_RICE_TUBE = 87;
Z_FRAME = 92;
...
// V48: Swirl pulse (Z-axis breathing)
swirl_pulse = 2 * sin(t * 360 * 0.3);

// Moon phase (VERY SLOW - 0.1x per user)
moon_phase_rot = t * 360 * 0.1;

// Lighthouse (SLOW - 0.3x per user)
lighthouse_rot = t * 360 * 0.3;

// === V48: ZONE-SPECIFIC WAVE PHASES ===
// Phase offsets create traveling wave illusion (right to left)
PHASE_ZONE_1_FAR = master_phase;           // Far: 0° (reference)
PHASE_ZONE_2_MID = master_phase + 30;      // Mid: +30°
PHASE_ZONE_3_BREAK = master_phase + 60;    // Break: +60°

// V48: Cypress wind sway
...
SHOW_GEARS = true;
SHOW_STARS = true;           // V48: Star twinkle system
SHOW_CLIFF = true;
...
SHOW_ZONE_WAVES = true;      // V48: New zone-based wave system
SHOW_FOUR_BAR = true;
SHOW_BIRD_WIRE = true;
SHOW_RICE_TUBE = true;
SHOW_FRAME = true;
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            V48: STAR TWINKLE SYSTEM (11 STARS)
// ═══════════════════════════════════════════════════════════════════════════════════════
// Star config: [x%, y%, radius, gear_speed, halo_speed, brightness]
STAR_CONFIG = [
...
module star_gear_v48(radius, rotation, brightness) {
...
module star_halo_v48(radius, rotation, brightness) {
    rotate([0, 0, rotation])
...
module star_twinkle_system_v48() {
    for (i = [0:len(STAR_CONFIG)-1]) {
        ...
        translate([TAB_W + x_pos, TAB_W + y_pos, 0]) {
            ...
            star_halo_v48(radius, halo_rot_star, brightness);

            translate([0, 0, Z_STAR_GEAR])
            star_gear_v48(radius, gear_rot_star, brightness);
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            V48: ZONE 1 - FAR OCEAN (Gentle Bob)
// ═══════════════════════════════════════════════════════════════════════════════════════
module zone_1_far_ocean_v48() {
    wave_area_start = ZONE_COMBINED_WAVES[0];  // 78
    wave_area_width = ZONE_COMBINED_WAVES[1] - wave_area_start;  // 224

    x_start = wave_area_start + wave_area_width * 0.70;
    zone_width = wave_area_width * 0.30;

    bob_1 = 2 * sin(PHASE_ZONE_1_FAR);
    bob_2 = 2 * sin(PHASE_ZONE_1_FAR + 15);
    bob_3 = 2 * sin(PHASE_ZONE_1_FAR + 30);
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            V48: ZONE 2 - MID OCEAN (Elliptical Orbit)
// ═══════════════════════════════════════════════════════════════════════════════════════
module zone_2_mid_ocean_v48() {
    wave_area_start = ZONE_COMBINED_WAVES[0];
    wave_area_width = ZONE_COMBINED_WAVES[1] - wave_area_start;

    x_start = wave_area_start + wave_area_width * 0.40;
    zone_width = wave_area_width * 0.30;

    drift = 3 * sin(PHASE_ZONE_2_MID * 0.8);
    bob_1 = 5 * sin(PHASE_ZONE_2_MID);
    bob_2 = 5 * sin(PHASE_ZONE_2_MID + 20);
    bob_3 = 5 * sin(PHASE_ZONE_2_MID + 40);
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            V48: ZONE 3 - ARTICULATED BREAKING WAVE
// ═══════════════════════════════════════════════════════════════════════════════════════
module zone_3_breaking_wave_v48() {
    wave_area_start = ZONE_COMBINED_WAVES[0];
    wave_area_width = ZONE_COMBINED_WAVES[1] - wave_area_start;
    ...
    crest_rise = phase_normalized < 120 ?
        (phase_normalized / 120) * 25 :
        phase_normalized < 180 ?
        25 - ((phase_normalized - 120) / 60) * 15 :
        10 * sin((phase_normalized - 180) * 2);

    curl_angle = phase_normalized < 100 ?
        (phase_normalized / 100) * 30 :
        phase_normalized < 160 ?
        30 + ((phase_normalized - 100) / 60) * 90 :
        120 - ((phase_normalized - 160) / 200) * 120;

    surge = 12 * sin(PHASE_ZONE_3_BREAK * 0.7);

    pivot_x = wave_area_start + 20 + surge;
    pivot_y = 8;
                ...
                rotate([crest_rise, 0, 0]) {
                    ...
                    translate([15, 18, Z_WAVE_LAYER_T]) {
                        rotate([curl_angle, 0, 0]) {
                            ...
                            spray_tips_v48(phase_normalized);
                        }
                    }
                }
            }
    ...
    if (phase_normalized > 100 && phase_normalized < 220) {
        ...
        foam_burst_v48(foam_intensity);
    }
}

module spray_tips_v48(phase) {
    detach = phase > 120 && phase < 200 ? (phase - 120) / 80 * 15 : 0;
...
module foam_burst_v48(intensity) {
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            FOUR-BAR MECHANISM (V48: Zone-Specific Cranks)
// ═══════════════════════════════════════════════════════════════════════════════════════
module four_bar_mechanism_v48() {
    camshaft_x = 100;
    camshaft_y = 35;

    // Zone crank throws
    ZONE_1_CRANK = 5;   // Far: minimal
    ZONE_2_CRANK = 8;   // Mid: medium
    ZONE_3_CRANK = 15;  // Break: maximum
    ...
    translate([TAB_W + camshaft_x, TAB_W + camshaft_y, Z_FOUR_BAR]) {
        ...
        // Zone 2 cranks (8mm throw) x2
        for (offset = [10, -15]) {
            translate([offset, 0, 0])
            rotate([PHASE_ZONE_2_MID + (offset < 0 ? 20 : 0), 0, 0])
            rotate([0, 90, 0]) {
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            COMPLETE GEAR TRAIN (From V47 - LOCKED)
// ═══════════════════════════════════════════════════════════════════════════════════════
module complete_gear_train() {
    translate([TAB_W, TAB_W, 0]) {
        ...
        // Idler chain (6 idlers to swirls)
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            SWIRL ASSEMBLIES (V48: With Z Pulse)
// ═══════════════════════════════════════════════════════════════════════════════════════
...
module swirl_assembly_small_v48() {
...
module wind_path() {
    ...
    translate([wp_x, wp_y, Z_WIND_PATH]) {
        ...
        difference() {
            offset(r=8)
            polygon([
                [15, 15], [wp_w * 0.3, 8], [wp_w * 0.6, 20],
                [wp_w - 15, 25], [wp_w - 8, wp_h * 0.5],
                [wp_w - 15, wp_h - 15], [wp_w * 0.5, wp_h - 8],
                [15, wp_h - 20], [8, wp_h * 0.6], [20, wp_h * 0.3], [15, 15]
            ]);
            translate([big_hole_x, big_hole_y]) circle(r=big_hole_r);
            translate([small_hole_x, small_hole_y]) circle(r=small_hole_r);
        }
    }
}

// ═══════════════════════════════════════════════════════════════════════════════════════
//                            BIRD WIRE SYSTEM (From V47)
// ═══════════════════════════════════════════════════════════════════════════════════════
...
// Star twinkle system (V48)
if (SHOW_STARS) star_twinkle_system_v48();

// Complete gear train
if (SHOW_GEARS) complete_gear_train();
...
// Cypress (V48: with wind sway)
if (SHOW_CYPRESS) cypress_v48();

// Moon
if (SHOW_MOON) moon_assembly();

// Wind path
if (SHOW_WIND_PATH) wind_path();

// Swirl assemblies (V48: with Z pulse)
if (SHOW_BIG_SWIRL) swirl_assembly_big_v48();
if (SHOW_SMALL_SWIRL) swirl_assembly_small_v48();

// Zone waves (V48: new physics-based system)
if (SHOW_ZONE_WAVES) {
    zone_1_far_ocean_v48();
    zone_2_mid_ocean_v48();
    zone_3_breaking_wave_v48();
}

// Four-bar mechanism (V48: zone-specific cranks)
if (SHOW_FOUR_BAR) four_bar_mechanism_v48();
...
// ═══════════════════════════════════════════════════════════════════════════════════════
echo("═══════════════════════════════════════════════════════════════════════════════════════");
echo("STARRY NIGHT V48 - COMPLETE ASSEMBLY");
echo("═══════════════════════════════════════════════════════════════════════════════════════");
echo("");
echo("V48 NEW FEATURES:");
echo("  ★ 3-Zone Wave System (Far/Mid/Breaking)");
echo("  ★ Articulated Breaking Wave with multi-segment curl");
echo("  ★ 11-Star Twinkle System with counter-rotating halos");
echo("  ★ Cypress wind sway (+/-", cypress_sway, "°)");
echo("  ★ Swirl Z-axis pulse (+/-", swirl_pulse, "mm)");
echo("");
echo("WAVE ZONE PHASES (Traveling Wave Illusion):");
echo("  Zone 1 (Far):   master +  0° = ", PHASE_ZONE_1_FAR % 360, "°");
echo("  Zone 2 (Mid):   master + 30° = ", PHASE_ZONE_2_MID % 360, "°");
echo("  Zone 3 (Break): master + 60° = ", PHASE_ZONE_3_BREAK % 360, "°");
echo("");
echo("ARTICULATED BREAKING WAVE MOTION:");
echo("  Base swell:   +/-8° tilt");
echo("  Rising crest: 0-25° lift");
echo("  Curling lip:  0-120° fold");
echo("  Spray detach: 15mm scatter");
echo("");
echo("STAR TWINKLE SYSTEM: 11 stars");
echo("  Gear speeds:  0.40x to 0.75x master");
echo("  Halo speeds:  0.28x to 0.58x (counter-rotating)");
echo("");
echo("PRESERVED FROM V47:");
...
echo("  ✓ Counter-rotating swirl discs");
echo("");
