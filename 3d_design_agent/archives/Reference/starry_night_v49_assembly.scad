// ═══════════════════════════════════════════════════════════════════════════════════════
//                    STARRY NIGHT V49 - ENHANCED WAVE MOTION ASSEMBLY
//                    Sinusoidal Motion + Harmonics + Easing Functions
//                    All V48 Components Preserved + Improved Wave Physics
// ═══════════════════════════════════════════════════════════════════════════════════════
// VERSION: V49
// BASE: V48 (Physics-Based Wave System)
// IMPROVEMENTS:
...
// ═══════════════════════════════════════════════════════════════════════════════════════
// USER CHOICE: Sinusoidal motion with harmonics enabled
...
// Enhanced sine with harmonics (organic feel)
...
Z_FOUR_BAR = 55;
Z_WAVE_START = 60;
Z_WAVE_LAYER_T = 5;     // V49: INCREASED from 4mm to prevent interference
Z_CYPRESS = 75;
Z_BIRD_WIRE = 82;
Z_RICE_TUBE = 87;
Z_FRAME = 92;
...
// ═══════════════════════════════════════════════════════════════════════════════════════
// Total phase span: 90° across 194mm wave area = 0.464°/mm
// Creates continuous traveling wave illusion rather than zone "stepping"
...
// V49: Graduated phase offsets (smoother than V48's fixed 30° jumps)
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                       V49: BREAKING WAVE PARAMETERS (UPDATED)
// ═══════════════════════════════════════════════════════════════════════════════════════
// User choice: Reduced curl from 120° to 90° for more realistic appearance

BREAKING_BASE_TILT_AMP = 8;     // Base swell tilt amplitude
BREAKING_PIVOT_OFFSET_X = 20;   // Pivot offset from wave area start

CURL_INITIAL_ANGLE = 30;        // Building curl phase
...
// ═══════════════════════════════════════════════════════════════════════════════════════
// Zone crank throws (V49: Zone 3 reduced for better Grashof margin)
ZONE_1_CRANK = 5;    // Far: minimal (5 + 38 = 43 < 50, margin=7)
ZONE_2_CRANK = 8;    // Mid: medium (8 + 34 = 42 < 50, margin=8)
ZONE_3_CRANK = 12;   // V49: REDUCED from 15mm (12 + 25 = 37 < 50, margin=13)

ZONE_1_OUTPUT = 2;   // +/-2mm bob
ZONE_2_DRIFT = 3;    // +/-3mm drift
ZONE_2_BOB = 5;      // +/-5mm bob
ZONE_2_DRIFT_FREQ = 0.95;  // V49: Improved from 0.8x
...
module star_gear_v49(radius, rotation, brightness) {
...
module star_halo_v49(radius, rotation, brightness) {
    rotate([0, 0, rotation])
...
module star_twinkle_system_v49() {
    for (i = [0:len(STAR_CONFIG)-1]) {
        ...
        translate([TAB_W + x_pos, TAB_W + y_pos, 0]) {
            ...
            star_halo_v49(radius, halo_rot_star, brightness);
            translate([0, 0, Z_STAR_GEAR])
            star_gear_v49(radius, gear_rot_star, brightness);
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            V49: ZONE 1 - FAR OCEAN (Sinusoidal + Harmonics)
// ═══════════════════════════════════════════════════════════════════════════════════════
module zone_1_far_ocean_v49() {
    x_start = WAVE_AREA_START + WAVE_AREA_WIDTH * 0.70;
    zone_width = WAVE_AREA_WIDTH * 0.30;

    // V49: Graduated phase offsets (18° spacing)
    ...
    // V49: Sinusoidal motion with harmonics
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            V49: ZONE 2 - MID OCEAN (Elliptical + Harmonics)
// ═══════════════════════════════════════════════════════════════════════════════════════
module zone_2_mid_ocean_v49() {
    x_start = WAVE_AREA_START + WAVE_AREA_WIDTH * 0.40;
    zone_width = WAVE_AREA_WIDTH * 0.30;

    // V49: Graduated phase offsets (12° spacing)
    ...
    // V49: Improved drift frequency (0.95x instead of 0.8x)
...
module zone_3_breaking_wave_v49() {
    ...
    // V49: Curl with EASING and REDUCED MAX (90° instead of 120°)
                ...
                rotate([crest_rise, 0, 0]) {
                    ...
                    translate([15, 18, Z_WAVE_LAYER_T]) {
                        rotate([curl_angle, 0, 0]) {
                            ...
                            spray_tips_v49(phase_normalized);
                        }
                    }
                }
            }
    ...
    if (phase_normalized > 100 && phase_normalized < 220) {
        ...
        foam_burst_v49(foam_intensity);
    }
}

module spray_tips_v49(phase) {
    // V49: Eased detachment
...
module foam_burst_v49(intensity) {
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                       V49: FOUR-BAR MECHANISM (Reduced Zone 3 Crank)
// ═══════════════════════════════════════════════════════════════════════════════════════
module four_bar_mechanism_v49() {
    camshaft_x = 100;
    camshaft_y = 35;
    ...
    translate([TAB_W + camshaft_x, TAB_W + camshaft_y, Z_FOUR_BAR]) {
        ...
        // Zone 2 cranks (8mm) x2 with V49 phase offsets
        for (offset = [10, -15]) {
            phase_offset = offset < 0 ? 12 : 0;  // V49: 12° offset (was 20°)
        ...
        // V49: Zone 3 crank (12mm - REDUCED from 15mm)
        translate([-40, 0, 0])
...
// ═══════════════════════════════════════════════════════════════════════════════════════
//                            REMAINING COMPONENTS (From V48)
// ═══════════════════════════════════════════════════════════════════════════════════════

// Complete gear train (unchanged from V48)
...
module swirl_assembly_small_v49() {
    ...
    rotate([0, 0, cypress_sway]) {
        for (layer = [0:3]) {
            ...
            polygon([
                ...
                [cy_w * 0.62, cy_h * 0.15], [cy_w * 0.75, cy_h * 0.28],
                [cy_w * 0.68, cy_h * 0.4], [cy_w * 0.82, cy_h * 0.52],
                [cy_w * 0.7, cy_h * 0.65], [cy_w * 0.78, cy_h * 0.78],
                [cy_w * 0.58, cy_h * 0.9], [cy_w * 0.5, cy_h],
                [cy_w * 0.42, cy_h * 0.9], [cy_w * 0.22, cy_h * 0.78],
                [cy_w * 0.3, cy_h * 0.65], [cy_w * 0.18, cy_h * 0.52],
                [cy_w * 0.32, cy_h * 0.4], [cy_w * 0.25, cy_h * 0.28],
                [cy_w * 0.38, cy_h * 0.15]
            ]);
        }
    }
...
module wind_path() {
    ...
    translate([wp_x, wp_y, Z_WIND_PATH]) {
        ...
        difference() {
            ...
            translate([big_hole_x, big_hole_y]) circle(r=37.5);
            translate([small_hole_x, small_hole_y]) circle(r=25);
        }
    }
}
...
if (SHOW_STARS) star_twinkle_system_v49();
if (SHOW_GEARS) complete_gear_train();
if (SHOW_CLIFF) cliff();
if (SHOW_LIGHTHOUSE) lighthouse();
if (SHOW_CYPRESS) cypress_v49();
if (SHOW_MOON) moon_assembly();
if (SHOW_WIND_PATH) wind_path();
if (SHOW_BIG_SWIRL) swirl_assembly_big_v49();
if (SHOW_SMALL_SWIRL) swirl_assembly_small_v49();

// V49: New wave system with sinusoidal motion + harmonics + easing
if (SHOW_ZONE_WAVES) {
    zone_1_far_ocean_v49();
    zone_2_mid_ocean_v49();
    zone_3_breaking_wave_v49();
}

// V49: Four-bar with reduced Zone 3 crank
if (SHOW_FOUR_BAR) four_bar_mechanism_v49();
...
// ═══════════════════════════════════════════════════════════════════════════════════════
echo("═══════════════════════════════════════════════════════════════════════════════════════");
echo("STARRY NIGHT V49 - ENHANCED WAVE MOTION ASSEMBLY");
echo("═══════════════════════════════════════════════════════════════════════════════════════");
echo("");
echo("V49 WAVE IMPROVEMENTS:");
...
echo("");
echo("PHASE PROGRESSION (V49 Graduated):");
