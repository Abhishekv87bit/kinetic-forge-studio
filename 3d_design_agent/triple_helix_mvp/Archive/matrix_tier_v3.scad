// =========================================================
// MATRIX TIER V3 — Hex-Parametric Prototype (Single Tier)
// =========================================================
// Everything derives from HEX_R (circumradius).
...
// =============================================
/* [Hex Tier] */
HEX_R         = 118;     // [40:1:150] circumradius — controls everything
WALL_MARGIN   = 8;       // [4:1:15] clearance each end for axle mounts

/* [Column Spacing] */
COL_PITCH     = 12;      // [8:1:30] column pitch along channel

/* [Pulley Stagger — prevents adjacent channels sharing X positions] */
// Alternating channels offset by half a pitch so strings from channel N
// never collide with channel N±1's pulleys when routed vertically.
STAGGER_HALF_PITCH = COL_PITCH / 2;  // 6mm half-pitch offset

/* [Channel Stacking — source of truth, other files sync to this] */
STACK_OFFSET  = 14.0;    // ⚠ sync: hex_frame, helix_cam, main_stack

/* [Mechanics] */
ECCENTRICITY  = 15.0;    // mm cam throw (±15mm)  ⚠ sync: hex_frame, helix_cam, main_stack

/* [Slider Offset — bias toward helix side] */
// SLIDER_BIAS: 0.0 = centered (protrudes equally both sides)
//              1.0 = fully one-sided (all protrusion toward helix)
// At 0.80 with ECC=15: rest offset = 12mm toward helix,
//   max helix-side protrusion = 27mm, arm-side = stays inside hex
SLIDER_BIAS        = 0.80;
...
// CHANNEL DIMENSIONS
// =============================================
WALL_THICKNESS = 2.5;                          // was 2.0 — 0.5mm backing behind rail slot
CH_GAP         = STACK_OFFSET - WALL_THICKNESS;  // 14 - 2.5 = 11.5mm (derived, not magic)

// HOUSING_HEIGHT — derived from pulley layout
FP_OD          = 8.0;
SP_OD          = 8.0;
_MIN_ROPE_GAP  = 2.0;                          // gap between FP and SP for rope passage
FP_ROW_Y       = (FP_OD + SP_OD) / 2 + _MIN_ROPE_GAP;  // (8+8)/2 + 2 = 10mm — derived
HOUSING_HEIGHT = 2 * FP_ROW_Y + FP_OD + 2;    // 2×10 + 8 + 2 = 30mm
...
// =============================================
// COLUMN COUNTS — per channel, culled to hex
// =============================================
// Base column X position (no stagger)
function _col_x_base(count, idx) =
    -((count - 1) / 2) * COL_PITCH + idx * COL_PITCH;

// Stagger offset for channel i: odd channels shift +half_pitch
function _ch_stagger(ch_idx) = (ch_idx % 2) * STAGGER_HALF_PITCH;

// Column X with stagger applied
...
C_ACRYLIC = [0.85, 0.85, 0.9, 0.6];  // flange ears (transparent)

// =============================================
// VERIFICATION ECHOES
// =============================================
echo(str("===== MATRIX TIER V3 — HEX PROTOTYPE ====="));
...
// (matches main_stack_v3.scad behavior)
...
rotate([90, 0, 0])
    matrix_tier_v3(_ch_disps);


// =========================================================
// MATRIX TIER V3 MODULE
// =========================================================
module matrix_tier_v3(ch_slider_disps) {
...
module _boundary_wall(length, rail_dir) {
    ...
    _stop = WALL_THICKNESS - PIP_Z_GAP;  // 2.5-0.3=2.2mm — deeper for impact resistance
    _win_w = min(WINDOW_WIDTH, length * 0.5);
    _win_z = WALL_THICKNESS + 4;  // through-cut
    ...
    difference() {
        ...
        // Rope routing window (centered, does NOT cut through rail)
...
// (Post notches are now cut inline in the matrix_tier_v3 module above)
