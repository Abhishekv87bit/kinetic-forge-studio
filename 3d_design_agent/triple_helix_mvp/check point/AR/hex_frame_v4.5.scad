// =========================================================
// HEX FRAME V5 — Hexagram Star Frame + Disc-Around-Shaft Cams
// =========================================================
// V5 PROTOTYPE: 75% scale, central shaft, 625ZZ bearings.
//
// CHANGES from previous hex_frame_v5:
//   - PLACEHOLDER MATRIX: ghost hex instead of full main_stack_v5
//   - FIXED: _shaft_crossing_frac now returns 0-1 fraction (was mm)
//   - EIFFEL TOWER LEGS: 3 pairs of splayed legs at stub vertices
//     Spread outward matching hexagonal theme, clear of blocks
//   - Leg cross-section matches arm beams but 1.5x thicker
//
// FRAME ARCHITECTURE:
...
FRAME_RING_H      = 12;
FRAME_RING_W      = 10;
FRAME_RING_R_IN   = HEX_R + 2;                          // 90.5mm
FRAME_RING_R_OUT  = FRAME_RING_R_IN + FRAME_RING_W;     // 100.5mm
...
LEDGE_WIDTH       = 6;
LEDGE_THICK       = 3;
LEDGE_R_IN        = FRAME_RING_R_IN - LEDGE_WIDTH;      // 84.5mm

/* [Hexagram Star — 6 main frame arms — PARALLEL FIX] */
V_ANGLE           = 70.87;    // PARALLELISM FIX
ARM_W             = 20;
ARM_H             = 14;
...
STUB_W            = 20;
STUB_H            = ARM_H;
STUB_R_START      = FRAME_RING_R_OUT - STUB_INWARD;     // 92.5mm
STUB_R_END        = FRAME_RING_R_OUT + STUB_LENGTH;     // 130.5mm
JUNCTION_R        = STUB_R_END + STUB_W / 2;            // 140.5mm
GUSSET_THICK      = 3;
ARM_CHAMFER       = 2;
STAR_TIP_R        = _STAR_RATIO * HEX_LONGEST_DIA;      // 265.5mm
HEXAGRAM_INNER_R  = STAR_TIP_R / sqrt(3);               // 153.3mm
CORRIDOR_GAP      = _CORRIDOR_GAP_CFG;                   // 58.5mm

// Helix radial position
_V_PUSH           = CORRIDOR_GAP / (2 * tan(30));       // 50.6mm
HELIX_R           = HEXAGRAM_INNER_R + _V_PUSH;         // 203.9mm
...
// =============================================
CARRIER_PLATE_T   = 20;
CARRIER_BRG_BORE  = FRAME_BRG_OD + 0.05;             // 16.05mm press-fit
CARRIER_WALL      = 4;
CARRIER_PLATE_W   = ARM_W;

_CARRIER_CLEARANCE = CARRIER_PLATE_T / 2 + 5;         // 15mm past crossing
...
    // ---- ECHOES ----
    echo(str("=== HEX FRAME V5 — 75% PROTOTYPE ==="));
    echo(str("V_ANGLE=", V_ANGLE, " (parallelism fix)"));
...
// =========================================================
// ALL CARRIER PLATES — 625ZZ bearing mounts at shaft crossing
// =========================================================
...
            _plate_h = _z_up - _z_lo + ARM_H;

            color(C_CARRIER)
            translate([_cross_xy[0], _cross_xy[1], _z_lo - ARM_H/2])
                rotate([0, 0, _sa])
                    difference() {
                        translate([-CARRIER_PLATE_T/2, -CARRIER_PLATE_W/2, 0])
                            cube([CARRIER_PLATE_T, CARRIER_PLATE_W, _plate_h]);

                        // 625ZZ bearing bore at shaft Z
                        _bore_z = _hz - (_z_lo - ARM_H/2);
                        translate([0, 0, _bore_z])
                            rotate([0, 90, 0])
                                cylinder(d = CARRIER_BRG_BORE, h = CARRIER_PLATE_T + 2,
                                         center = true, $fn = 32);
                    }

            // 625ZZ bearing visualization
            color(C_BEARING)
...
// =========================================================
// DAMPENER ARRAY — bar pairs across corridors
// =========================================================
DAMPENER_FRAC     = 0.50;
