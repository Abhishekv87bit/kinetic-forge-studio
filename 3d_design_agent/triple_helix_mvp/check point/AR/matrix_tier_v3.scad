...
function culled_col_count(ch_idx) =
    ...
    let(valid = [for (j = [0:max(0, raw-1)])
        if (col_inside_hex(col_x(raw, j), d)) 1])
    len(valid);
...
function _culled_span(ch_idx) =
    let(d = CH_OFFSETS[ch_idx],
        len = CH_LENS[ch_idx],
        raw = raw_col_count(len),
        // Find outermost culled column positions
        cols = [for (j = [0:max(0, raw-1)]) if (col_inside_hex(col_x(raw, j), d)) col_x(raw, j)])
    len(cols) == 0 ? 0 :
...
CH_S_LENS = [for (i = [0:NUM_CHANNELS-1])
    (COL_COUNTS[i] > 0) ?
        (COL_COUNTS[i] - 1) * COL_PITCH + SLIDER_MARGIN_HELIX + SLIDER_MARGIN_ARM : 0
];
