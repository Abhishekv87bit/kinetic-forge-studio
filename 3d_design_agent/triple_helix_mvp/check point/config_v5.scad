...
//   - Each disc: hub+web+lobe profile on central shaft
//   - 6800ZZ cam bearing on each disc lobe (same as V4)
//   - Follower ring on each bearing outer race (cable attachment)
//   - 625ZZ frame bearings (5x16x5) in carrier plates
//   - D-flat shaft + M3 set screw on each disc hub
//   - No FINAL_SCALE toggle — V5 is prototype only
//
// DECOUPLED SPACING (the "middle way"):
//   Matrix channels spaced at STACK_OFFSET = 12mm (tight matrix, 13 ch)
...
//   This gives 13 channels with comfortable disc geometry.
//
// Architecture:
...
/* [Channel Stacking] */
STACK_OFFSET  = 12.0;      // channel center-to-center in matrix (12mm → 13 channels)

// Channel count (derived from hex geometry)
function _half_count() = floor((HEX_FF/2 - STACK_OFFSET/2) / STACK_OFFSET);
NUM_CHANNELS = 2 * _half_count() + 1;  // 13 at HEX_R=98, STACK_OFFSET=12
...
// =============================================
// HELIX CAM — Central Shaft Disc Parameters (V5)
// =============================================
NUM_CAMS       = NUM_CHANNELS;                        // 13 (1 cam per channel)
...
HELIX_LENGTH      = NUM_CAMS * AXIAL_PITCH;            // 182mm (13 × 14)
...
SHAFT_TOTAL_LENGTH   = HELIX_LENGTH + 2 * (SHAFT_EXT_TO_CARRIER + SHAFT_EXT_BEYOND);  // 452mm
...
_CORRIDOR_GAP_CFG = 65.0;    // widened corridor for 13-cam helix (follower OD=58mm + clearance)
