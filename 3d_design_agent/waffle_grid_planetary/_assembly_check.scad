// Assembly check: carrier_2 (parametric) + carrier_3 + pinions
// Shows how they mesh together
...
// Parametric carrier_2
module carrier2_parametric() {
    ...
    difference() {
        union() {
            ...
            cylinder(d=CAR_HUB_OD, h=CAR_HUB_ZTOP - CAR_CAP_ZTOP);

            translate([0, 0, CAR_CAP_ZBOT])
            cylinder(d=CAR_CAP_OD, h=CAR_CAP_ZTOP - CAR_CAP_ZBOT);
        }
        translate([0, 0, CAR_CAP_ZBOT - 0.1])
        cylinder(d=CAR_HUB_ID, h=CAR_PLATE_ZTOP - CAR_CAP_ZBOT + 0.2);
...
// Flip to match viewport orientation
rotate([180, 0, 0]) {
    // Carrier 2 (parametric)
    carrier2_parametric();

    // Carrier 3 cages x3 (STL, semi-transparent green)
    for (i = [0:2])
        color("green", 0.4)
        rotate([0, 0, i * 120])
