...
translate([-50, 0, 0]) color("orange") import(str(STL_DIR, "planetary_2.stl"), convexity=4);
// Use the same profile from main file
CAR_PROFILE_PTS = [
...
translate([50, 0, 0]) {
    ...
    difference() {
        union() {
            translate([0, 0, -9])
            linear_extrude(height=9) polygon(CAR_PROFILE_PTS);
            translate([0, 0, -34.25])
            cylinder(d=33, h=34.25, $fn=64);
        }
        translate([0, 0, -34.35]) cylinder(d=26, h=34.55, $fn=64);
        for (i = [0:2]) {
            rotate([0, 0, i * 120]) translate([31.5, 0, -9.1]) cylinder(d=5, h=10, $fn=24);
            rotate([0, 0, i * 120 + 60]) translate([29.5, 0, -9.1]) cylinder(d=5, h=10, $fn=24);
        }
    }
}
