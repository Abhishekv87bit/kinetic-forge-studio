// Overlay: parametric carrier_2 (green) directly on top of original STL (orange)
// Both at same position — any mismatch will show as color bleed

STL_DIR = "../gears/automatic-transmission-double-planetary-gearset-ravigneaux-model_files/";

// Import all the carrier params from main file
PO_ORBIT = 31.5;
PI_ORBIT = 29.5;  // old value for reference
PI_ORBIT_ACTUAL = 27.44;
PI_ANG_OFFSET = 71.5;
...
// === ORIGINAL STL (orange, semi-transparent) ===
color("orange", 0.5)
import(str(STL_DIR, "planetary_2.stl"), convexity=4);

// === PARAMETRIC CARRIER (green, semi-transparent) ===
color("lime", 0.5)
difference() {
    union() {
        // Star plate
    ...
    // Pi pin holes (corrected orbit and angle)
    for (i = [0:2])
...
// === Reference markers for Pi pin positions ===
// Red dots at expected Pi positions
for (i = [0:2]) {
    ang = i * 120 + PI_ANG_OFFSET;
    x = PI_ORBIT_ACTUAL * cos(ang);
    y = PI_ORBIT_ACTUAL * sin(ang);
    color("red") translate([x, y, -1]) cylinder(d=2, h=3, $fn=16);
}

// Blue dots at Po positions
for (i = [0:2]) {
    ang = i * 120;
    x = PO_ORBIT * cos(ang);
    y = PO_ORBIT * sin(ang);
    color("blue") translate([x, y, -1]) cylinder(d=2, h=3, $fn=16);
}
