/*
 * BEARING BLOCK LEFT - Left Side Bearing Mount
 *
 * Printable part: 1x
...
module block_body() {
    // Main block - taller to get shaft at right height
...
module bearing_pocket() {
    ...
    // Bearing sits at top of block, shaft passes through center
...
echo("=== BEARING BLOCK LEFT ===");
echo(str("Block size: ", BEARING_BLOCK_WIDTH, "x", BEARING_BLOCK_DEPTH, "x", BEARING_BLOCK_HEIGHT, "mm"));
echo(str("Bearing pocket: ", BEARING_POCKET_DIA, "mm dia x ", BEARING_POCKET_DEPTH, "mm deep"));
echo(str("Shaft height from base: ", BEARING_BLOCK_HEIGHT, "mm"));
echo("");
echo("Print quantity: 1");
echo("Print orientation: Base down");
echo("Post-process: Press 608 bearing into pocket from +X side");
